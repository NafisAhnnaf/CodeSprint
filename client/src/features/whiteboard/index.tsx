...
import {fetchAllWhiteboards, fetchWhiteboardById, saveWhiteboard} from "./whiteboardRequests";
...
export default function Home() {
  const whiteboard_api = import.meta.env.VITE_WHITEBOARD_API;
  ...
  const getExplanation = async () => {
    ...
    try {
      const response = await axios({
        method: "post",
        url: `${whiteboard_api}/calculate/explain`,
        data: {
          image: canvas.toDataURL("image/png"),
  ...
  // Functions Added by Nafis:
  const [board, setBoard] = useState({drawingActions:"", actionIndex: "", answers: "", title:""});
  const [allBoards, setAllBoards] =  useState([]);
  useEffect(()=>{   
     const allBoards = fetchAllWhiteboards();
     if(allBoards.length() === null ){
      console.log("No history found");
     }
  }, []);
 
  const handleFetchBoardById = ()=>{
    fetchWhiteboardById(boardId);
  }
